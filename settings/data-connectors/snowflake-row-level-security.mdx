...
<Note>
**Variable names are case-sensitive when read.** Snowflake stores session variables uppercased, so `SESSION_CONTEXT('EMAIL')` returns the value while `SESSION_CONTEXT('email')` returns `NULL`. The same applies to `GETVARIABLE`. The `SET` statement itself is case-insensitive.
</Note>
...
If the user attribute you want to filter on isn't a column on the protected table — for example, a separate table maps users to the allowed values — see [Option 3](#option-3-filter-via-a-user-to-row-mapping-table) below for the lookup pattern.
...
CREATE OR REPLACE FUNCTION BLUEPEAK_ANALYTICS.SECURITY.my_departments()
    ...
    $$
        SELECT ARRAY_AGG(DEPARTMENT)
         ...
         WHERE USER_EMAIL = SESSION_CONTEXT('EMAIL')
    $$;
...
SET email = 'dan@bluepeak.example';
...
| Identity carried into the policy | Custom user attribute (for example `USER_ROLE`) | Active Snowflake role | Custom user attribute (typically email), looked up in a mapping table |
