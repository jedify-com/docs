...
| `JEDIFY_USER_ROLE` | The user's Jedify role. One of `User`, `Data Expert`, or `Admin` (see [Role-Based Access Control](../rbac) for what each grants in Jedify). |
...
Use this option when each row already carries the identity of the user it belongs to (an email column, an owner column, and so on). The policy compares that column to `SESSION_CONTEXT('JEDIFY_USER_EMAIL')`, with a bypass for users whose Jedify role is `Admin`.


The example below filters `USER_EVENTS` so each end-user only sees rows where the `USER_EMAIL` column matches their own email. Jedify Admins see everything.
...
CREATE OR REPLACE ROW ACCESS POLICY BLUEPEAK_ANALYTICS.SECURITY.rap_own_events
    ...
    RETURNS BOOLEAN
    -> CASE
           -- 1. Jedify Admins see everything
...
Simulate a Jedify session as `bob` (a regular user) and then as an Admin:
