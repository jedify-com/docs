...
| `JEDIFY_USER_ROLE` | The user's organizational role (for example `analyst`, `manager`, `admin`). Values are organization-defined and sourced from the user's Jedify profile or SSO claims. |
| `JEDIFY_USER_SNOWFLAKE_ROLE` | The Snowflake role Jedify activates with `USE ROLE` before the query, when a per-user Snowflake role is configured. `NULL` when no per-user role is set — in that case the query runs as Jedify's default service role. Inside a policy, read the actual session role with `CURRENT_ROLE()`; this variable is useful for auditing or debugging the intended mapping. |
...
Use this option when you want Jedify to drive row filtering from the user's organizational role (`JEDIFY_USER_ROLE`) without creating one Snowflake role per access group.


### 1. Set the user role on each Jedify user


Set the user role on each Jedify user — `manager`, `analyst`, or `admin` in this example — so Jedify forwards it as `JEDIFY_USER_ROLE` on every query. The role must be supplied at SSO sign-in, so **this approach assumes SSO is configured** for your Jedify users.
...
CREATE OR REPLACE ROW ACCESS POLICY BLUEPEAK_ANALYTICS.SECURITY.rap_user_role_filter
    ...
    -> CASE
           ...
           WHEN SESSION_CONTEXT('JEDIFY_USER_EMAIL') = 'admin@bluepeak.example'
               THEN TRUE
           -- 2. Admin role bypass
           WHEN SESSION_CONTEXT('JEDIFY_USER_ROLE') = 'admin'
               THEN TRUE
           -- 3. Otherwise: viewer's role must match the row's USER_ROLE
           WHEN SESSION_CONTEXT('JEDIFY_USER_ROLE') = user_role
               THEN TRUE
           ELSE FALSE
       END;
```
...
SET JEDIFY_USER_EMAIL = 'bob@bluepeak.example';
SET JEDIFY_USER_ROLE  = 'analyst';
SELECT * FROM BLUEPEAK_ANALYTICS.PRODUCT.USER_EVENTS;

-- As the admin email — all rows
SET JEDIFY_USER_EMAIL = 'admin@bluepeak.example';
SET JEDIFY_USER_ROLE  = 'admin';
SELECT * FROM BLUEPEAK_ANALYTICS.PRODUCT.USER_EVENTS;
```
...
| Identity carried into the policy | `SESSION_CONTEXT('JEDIFY_USER_ROLE')` (or another session variable) | `CURRENT_ROLE()` | `SESSION_CONTEXT('JEDIFY_USER_EMAIL')` looked up in a mapping table |
