...
1. **Session variables.** Jedify reads custom attributes from the user's profile (for example `USER_ROLE` or `USER_DEPARTMENT`) and sets them as Snowflake session variables before running the query. The policy reads them via `SESSION_CONTEXT('<name>')`.
2. **Snowflake role.** Jedify can switch the session to a specific Snowflake role per user (via the `snowflake_role` user attribute) before running the query. The policy reads `CURRENT_ROLE()`.


Both rely on user attributes configured in Jedify and supplied at login (either directly or mapped from your identity provider). You can use either approach in isolation or combine them in a single policy.


### Session variables and user attributes


Every Jedify user has a `snowflake_params` custom attribute. It is a semicolon-separated list of variables to forward to Snowflake at the start of each session.


For example, if a user's `snowflake_params` is:


```
email;USER_ROLE=analyst;
```


Jedify runs, before each query on behalf of that user:
...
- `email` is forwarded by name from the user's account.
- `USER_ROLE` (and any other `KEY=value` entry) is forwarded with the literal value defined on the user.
...
If the user has a `snowflake_role` custom attribute, Jedify executes `USE ROLE <role>` before each query. The Jedify service user must already be granted that role (see [Snowflake Data Connector](./snowflake)). Policies can then read `CURRENT_ROLE()` directly.
...
On each user, set the `snowflake_params` custom attribute so that `USER_ROLE` is forwarded at session start:
...
Use values like `manager`, `analyst`, or `admin` to match the labels you'll reference in the policy.
...
On each Jedify user, set the `snowflake_role` custom attribute to one of `BLUEPEAK_SALES_ROLE`, `BLUEPEAK_MARKETING_ROLE`, or `BLUEPEAK_FINANCE_ROLE`. Jedify will issue `USE ROLE <role>` before each query on that user's behalf.
...
| Who is the filter "owned" by | Jedify user attribute (`snowflake_params`) | Snowflake role hierarchy (`snowflake_role`) |
