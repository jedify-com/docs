...
2. **Using an API key?** `--builder` is ignored — the mode comes from the key, and keys created in the app are Asker. Set it in the server URL instead — `"REMOTE_MCP_URL": "https://be.jedify.com/mcp/message?mode=builder"` (see [Select a mode with an API key](/mcp/connect#select-a-mode-with-an-api-key)).
...
3. **Using an API key?** `--editor` is ignored — the mode comes from the key, and keys created in the app are Asker. Set it in the server URL instead — `"REMOTE_MCP_URL": "https://be.jedify.com/mcp/message?mode=editor"` (see [Select a mode with an API key](/mcp/connect#select-a-mode-with-an-api-key)).
