...
1. **Check the connector is enabled**: in Claude, open **Settings → Connectors** — the Jedify connector for your mode should be listed and connected. If it's missing, search the directory for **Jedify** (Asker: [claude.ai/directory/jedify](https://claude.ai/directory/jedify)) or ask your workspace admin to enable the Jedify connectors for you.
2. **Hosted agent or other MCP client?** Check the server URL: it must be `https://be.jedify.com/mcp`, with `?mode=editor` or `?mode=builder` for the other modes — no other path.
3. **Reconnect**: disconnect the connector, connect it again, and sign in.
...
1. **Confirm Builder mode**: in Claude, connect the **Jedify Production Builder** connector — **Jedify Production** is Asker only. For hosted agents, the server URL must be `https://be.jedify.com/mcp?mode=builder`; the mode comes from the URL, whether you sign in or use an API key (see [Switching modes](/mcp/modes#switching-modes)).
...
**Solution**: in Claude, connect the **Jedify Production Editor** connector — **Jedify Production** is Asker only. For hosted agents, the server URL must be `https://be.jedify.com/mcp?mode=editor`; the mode comes from the URL, whether you sign in or use an API key (see [Switching modes](/mcp/modes#switching-modes)).
