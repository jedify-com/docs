...
1. **Check the connector is enabled**: in Claude, open **Settings → Connectors** — Jedify should be listed and connected. If it's missing, connect it from [claude.ai/directory/jedify](https://claude.ai/directory/jedify) or ask your workspace admin to enable it for you.
...
1. **Confirm Builder mode**: Builder mode isn't available through the Claude connector, which runs in Asker mode. For hosted agents and other MCP clients, the server URL must be `https://be.jedify.com/mcp?mode=builder` — the mode comes from the URL, whether you sign in or use an API key (see [Switching modes](/mcp/modes#switching-modes)).
...
**Solution**: Editor mode isn't available through the Claude connector, which runs in Asker mode. For hosted agents and other MCP clients, the server URL must be `https://be.jedify.com/mcp?mode=editor` — the mode comes from the URL, whether you sign in or use an API key (see [Switching modes](/mcp/modes#switching-modes)).
