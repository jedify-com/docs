...
1. **Check the connector is enabled**: in Claude, click **+ → Connectors** — Jedify should be listed and connected. If it's missing, connect it from [claude.ai/directory/jedify](https://claude.ai/directory/jedify) or ask your workspace admin to enable it for you.
...
1. **Confirm Builder mode**: the server URL must be `https://be.jedify.com/mcp?mode=builder` — the mode comes from the URL, whether you sign in or use an API key (see [Switching modes](/mcp/modes#switching-modes)). In Claude, Builder mode is its own custom connector — the directory connector is Asker only.
...
**Solution**: the server URL must be `https://be.jedify.com/mcp?mode=editor` — the mode comes from the URL, whether you sign in or use an API key (see [Switching modes](/mcp/modes#switching-modes)). In Claude, Editor mode is its own custom connector — the directory connector is Asker only.
