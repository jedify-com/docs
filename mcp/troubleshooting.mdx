...
2. **Using an API key?** A key runs in Asker mode on its own. Add `?mode=builder` to the server URL — `"REMOTE_MCP_URL": "https://be.jedify.com/mcp/message?mode=builder"` for the local proxy, `https://be.jedify.com/mcp?mode=builder` for a hosted agent.
...
2. **Using an API key?** A key runs in Asker mode on its own. Add `?mode=editor` to the server URL — `"REMOTE_MCP_URL": "https://be.jedify.com/mcp/message?mode=editor"` for the local proxy, `https://be.jedify.com/mcp?mode=editor` for a hosted agent.
