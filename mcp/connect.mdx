...
<Warning>
A key created in the app carries **Asker** mode on its own, regardless of your role. To use Editor or Builder tools with a key, add `REMOTE_MCP_URL` alongside it as shown in [Restricting the proxy to one mode](#restricting-the-proxy-to-one-mode) — the URL parameter takes precedence over the key's own mode, so the same block works with or without a key.
</Warning>
