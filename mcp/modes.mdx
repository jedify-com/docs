...
**In Claude**, each mode is a separate connector — **Jedify Production** (Asker), **Jedify Production Editor**, and **Jedify Production Builder**. Connect the one you need; there is nothing to configure. See [Connect](/mcp/connect).

**Everywhere else** — hosted agents and other MCP clients — the mode is selected in the server URL. Asker is the default; no parameter needed.
...
The URL parameter takes precedence over everything else: an API key created in the app carries Asker mode on its own, and `?mode=` overrides it.
