...
In Claude, each mode is its own connector — add one per mode you need. For hosted agents, set the URL in the platform's MCP settings. The URL parameter takes precedence over everything else: an API key created in the app carries Asker mode on its own, and `?mode=` overrides it.
