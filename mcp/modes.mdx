...
Asker mode is the default — no flag needed. How you select another mode depends on how you authenticate.

**Browser sign-in (local proxy).** Add the matching flag to the proxy `args`:
...
**API key — local proxy or hosted agent.** The flag is ignored: the mode comes from the key, and keys created in the app carry Asker mode. Select the mode in the server URL instead — `https://be.jedify.com/mcp/message?mode=editor` or `?mode=builder`. For the local proxy, set that URL as `REMOTE_MCP_URL` in the `env` block. The URL parameter takes precedence over every other setting, so it works with the browser sign-in too.
