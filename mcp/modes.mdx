...
Asker is the default mode. The **Claude connector runs in Asker mode only** — Editor and Builder are not yet available through it.

For **hosted agents and other MCP clients**, the mode is selected in the server URL:
...
The URL parameter takes precedence over everything else: an API key created in the app carries Asker mode on its own, and `?mode=` overrides it. See [Connect](/mcp/connect) for the full setup.
